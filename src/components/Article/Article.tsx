...
import { IArticleProps } from '../models/IArticle';
...
const splicedText = (text: string, lengthValue: number) => {
  let shortenText;
  if (!text) {
    return text;
  }
...
const Article: FC<IArticleProps> = ({ article, isDetailed }) => {
  const {
    slug,
    title,
    description,
  ...
  const [likedCount, setLikedCount] = useState<number>(favoritesCount);
  const [likeTrigger, setLikeTrigger] = useState<boolean | string | null>(
  ...
  const getUser = JSON.parse(user as string);
